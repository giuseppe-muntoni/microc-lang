...
let local_entry_test _ = 
  ...
  let scope = Symbol_table.add_entry "x" (Symbol.Var(Types.PrimitiveType Types.VoidType, false)) scope in 
  let x = Symbol_table.lookup "x" scope in 
  assert_equal (Symbol.Var(Types.PrimitiveType Types.VoidType, false)) x
...
let shadow_lookup_test _ = 
  let global_var = Symbol.Var(Types.PrimitiveType Types.VoidType, true) in 
  let global_scope = Symbol_table.empty_table in 
  let global_scope = Symbol_table.add_entry "x" global_var global_scope in 
  let local_var = Symbol.Var(Types.PrimitiveType Types.BoolType, false) in 
...
let entry_not_found_test _ = 
  let global_scope = Symbol_table.empty_table in
  let local_scope = Symbol_table.begin_block global_scope in 
  let local_scope = Symbol_table.add_entry "y" (Symbol.Var(Types.PrimitiveType Types.CharType, false)) local_scope in 
...
let duplicate_entry_test _ = 
  ...
  let local_scope = Symbol_table.begin_block global_scope in 
  let local_scope = Symbol_table.add_entry "y" (Symbol.Var(Types.PrimitiveType Types.CharType, false)) local_scope in 
  assert_raises (Symbol_table.DuplicateEntry("y")) (fun _ -> Symbol_table.add_entry "y" (Symbol.Var(Types.PrimitiveType Types.BoolType, false)) local_scope) 
