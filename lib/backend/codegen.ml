...
let rec generate_access_code access llvm_context builder llvalues_table symbol_table = 
  ...
  | Ast.AccVar id ->
    ...
    (address, value)
  | Ast.AccDeref expr -> 
    let address = generate_expr_code expr llvm_context builder llvalues_table symbol_table in 
    let value = Llvm.build_load address "derefvalue" builder in 
    (address, value)
  | Ast.AccIndex(access, expr) -> 
    let (address_of_arr, value_of_arr) = generate_access_code access llvm_context builder llvalues_table symbol_table in 
    let index = generate_expr_code expr llvm_context builder llvalues_table symbol_table in 
...
and generate_expr_code expr llvm_context builder llvalues_table symbol_table = 
  ...
  | Ast.Assign(access, expr) -> 
    ...
    let new_value = generate_expr_code expr llvm_context builder llvalues_table symbol_table in 
  ...
  | Ast.Addr(access) -> 
    ...
    address 
  | Ast.ILiteral(i) ->
    Llvm.const_int (Llvm.i32_type llvm_context) i
  | Ast.CLiteral(c) -> 
    Llvm.const_int (Llvm.i8_type llvm_context) (Char.code c)
  | Ast.BLiteral(b) -> 
    Llvm.const_int (Llvm.i1_type llvm_context) (Bool.to_int b)
  | Ast.SLiteral(s) ->
    Llvm.build_global_stringptr s "str" builder
  | Ast.FLiteral(f) -> 
    Llvm.const_float (Llvm.float_type llvm_context) f
  | Ast.UnaryOp(Ast.Not, expr) -> 
    let value = generate_expr_code expr llvm_context builder llvalues_table symbol_table in 
    Llvm.build_not value "not" builder
  | Ast.UnaryOp(Ast.Neg, expr) -> 
    let value = generate_expr_code expr llvm_context builder llvalues_table symbol_table in 
    Llvm.build_neg value "neg" builder
  | Ast.BinaryOp(op, expr1, expr2) -> (
    let op1 = generate_expr_code expr1 llvm_context builder llvalues_table symbol_table in 
    let op2 = generate_expr_code expr2 llvm_context builder llvalues_table symbol_table in 
    match op with 
    | Ast.Add -> (
      if (is_float_op op1 op2 llvm_context) then 
        Llvm.build_fadd op1 op2 "fadd" builder
      else
        Llvm.build_add op1 op2 "add" builder)
    | Ast.Sub -> (
      if (is_float_op op1 op2 llvm_context) then 
        Llvm.build_fsub op1 op2 "fsub" builder
      else
        Llvm.build_sub op1 op2 "sub" builder)
    | Ast.Mult -> (
      if (is_float_op op1 op2 llvm_context) then 
        Llvm.build_fmul op1 op2 "fmult" builder
      else
        Llvm.build_mul op1 op2 "mult" builder)
    | Ast.Div -> (
      if (is_float_op op1 op2 llvm_context) then 
        Llvm.build_fdiv op1 op2 "fdiv" builder
      else
        Llvm.build_udiv op1 op2 "div" builder)
    | Ast.Mod -> 
      Llvm.build_urem op1 op2 "mod" builder
    | Ast.Equal -> (
      if (is_float_op op1 op2 llvm_context) then 
        Llvm.build_fcmp Llvm.Fcmp.Oeq op1 op2 "oeq" builder
      else
        Llvm.build_icmp Llvm.Icmp.Eq op1 op2 "eq" builder)
    | Ast.Neq -> (
      if (is_float_op op1 op2 llvm_context) then 
        Llvm.build_fcmp Llvm.Fcmp.One op1 op2 "one" builder
      else
        Llvm.build_icmp Llvm.Icmp.Ne op1 op2 "ne" builder)
    | Ast.Less -> (
      if (is_float_op op1 op2 llvm_context) then 
        Llvm.build_fcmp Llvm.Fcmp.Olt op1 op2 "olt" builder
      else
        Llvm.build_icmp Llvm.Icmp.Slt op1 op2 "slt" builder)
    | Ast.Leq -> (
      if (is_float_op op1 op2 llvm_context) then 
        Llvm.build_fcmp Llvm.Fcmp.Ole op1 op2 "ole" builder
      else
        Llvm.build_icmp Llvm.Icmp.Sle op1 op2 "sle" builder)
    | Ast.Greater -> (
      if (is_float_op op1 op2 llvm_context) then 
        Llvm.build_fcmp Llvm.Fcmp.Ogt op1 op2 "ogt" builder
      else
        Llvm.build_icmp Llvm.Icmp.Sgt op1 op2 "sgt" builder)
    | Ast.Geq -> (
      if (is_float_op op1 op2 llvm_context) then 
        Llvm.build_fcmp Llvm.Fcmp.Oge op1 op2 "oge" builder
      else
        Llvm.build_icmp Llvm.Icmp.Sge op1 op2 "sge" builder)
    | Ast.And -> 
      Llvm.build_and op1 op2 "and" builder
    | Ast.Or ->
      Llvm.build_or op1 op2 "or" builder
    )
  | Ast.Call(id, actuals) -> 
    let fun_llvalue = Symbol_table.lookup id llvalues_table in 
    let actuals = List.fold_left (fun others actual ->
      let actual = generate_expr_code actual llvm_context builder llvalues_table symbol_table in 
      actual :: others
    ) [] actuals in 
    let actuals = List.rev actuals in 
    Llvm.build_call fun_llvalue (Array.of_list actuals) "" builder
...
let rec generate_stmt_code stmt llvm_context bblock builder llvalues_table symbol_table fun_llvalue return_basic_block =
  ...
  | Ast.Expr expr ->
    let _ = generate_expr_code expr llvm_context builder llvalues_table symbol_table in 
    (bblock, builder)
  | Ast.Return(Some(return_expr)) ->
    let ret_val_addr = Symbol_table.lookup "ret" llvalues_table in 
    let ret_val = generate_expr_code return_expr llvm_context builder llvalues_table symbol_table in 
...
and generate_if_code (guard, then_stmt, else_stmt) llvm_context builder llvalues_table symbol_table fun_llvalue return_basic_block =
  ...
  let guard_val = generate_expr_code guard llvm_context builder llvalues_table symbol_table in 
...
and generate_while_code (guard, body) llvm_context builder llvalues_table symbol_table fun_llvalue return_basic_block = 
  ...
  let guard_val = generate_expr_code guard llvm_context while_guard_builder llvalues_table symbol_table in 
...
let generate_rts_functions global_symbol_table global_llvalues_table llvm_module llvm_context =
  ...
  Symbol_table.add_entry "getint" fun_llvalue llvalues_table
